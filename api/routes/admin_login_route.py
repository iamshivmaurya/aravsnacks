...
async def signup(request: SignupRequestAdmin, db: Session = Depends(get_db)):
    # Validate phone number
    if not request.user_name or (request.password):
       raise HTTPException(
            status_code=400,
            detail="please enter currect name or password."
       )

    # Check if customer already exists
    existing_customer = db.query(AdminLogin).filter(AdminLogin.user_name == request.user_name and AdminLogin.password == request.password ).first()
